 ...
 * import java.util.Map; import jakarta.servlet.ServletContext; import
 ...
 * jakarta.servlet.http.HttpServletResponse; import utils.BoardPage;
 * 
 * @WebServlet("/userlist.do") public class UserListController extends
 * HttpServlet { private static final long serialVersionUID = 1L;
 * 
 * @Override protected void doGet(HttpServletRequest req, HttpServletResponse
 * resp) throws ServletException, IOException { UserBoardDAO dao = new
 * UserBoardDAO();
 * 
 * Map<String, Object> map = new HashMap<>();
 * 
 * String searchField = req.getParameter("searchField"); String searchWord =
 * req.getParameter("searchWord");
 * 
 * if (searchWord != null && !searchWord.isEmpty()) { map.put("searchField",
 * searchField); map.put("searchWord", searchWord); }
 * 
 ...
 * 
 * int pageNum = 1; String pageTemp = req.getParameter("pageNum"); if (pageTemp
 * != null && !pageTemp.isEmpty()) { pageNum = Integer.parseInt(pageTemp); }
 * 
 * int start = (pageNum - 1) * pageSize + 1; int end = pageNum * pageSize;
 * map.put("start", start); map.put("end", end);
 * 
 * List<UserBoardDTO> boardLists = dao.selectListPage(map); dao.close();
 * 
 * String pagingImg = BoardPage.pagingStr(totalCount, pageSize, blockPage,
 * pageNum, "userlist.do"); map.put("pagingImg", pagingImg);
 * map.put("totalCount", totalCount); map.put("pageSize", pageSize);
